...
class CVW2PDF:
    ...
    def getDataWeb(self):
        ...
        taskId = re.search("ilovepdfConfig.taskId = '(.*?)'",
                           reqWeb.text).group(1)
    ...
    def uploadFileToServer(self):
        ...
        fileWhileUpload = (self.nameDocs, self.url)

        self.headers['authorization'] = 'Bearer '+loadJson['bearerToken']

        reqPostUpload = req.post("https:"+loadJson['servers']+"/v1/upload", files={
                                 'file': fileWhileUpload}, data=postData, headers=self.headers)
    ...
    def processConvertFile(self):
        ...
        reqPostProcess = req.post(
            "https:"+loadJson2['server']+"/v1/process", data=postData, headers=self.headers)
        decodeJson = json.loads(reqPostProcess.text)

        decodeJson['download_url'] = "https:" + \
            loadJson2['server'] + "/v1/download/" + loadJson2['taskId']

        decodeJson['success'] = True
        decodeJson['msg'] = "Success convert file"
        return json.dumps(decodeJson)

    def executeProcess(self):
        try:
            return self.processConvertFile()
        except:
            result = {
                'success': False,
